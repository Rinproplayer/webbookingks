...
export default function ImageUploader({ 
  // API 1: single or multiple with value / values
  value = '', 
  onChange, 
  label = '',
  multiple = false,
  values = [],
  onMultipleChange,
  // API 2: used in AdminDashboard with images / maxImages / placeholder
  images = null,
  maxImages = 1,
  placeholder = ''
}) {
  ...
  // Normalize current images based on which prop was supplied
  const isImagesMode = images !== null;
  const isMulti = multiple || (isImagesMode && maxImages > 1);

  let currentList = [];
  if (isImagesMode) {
    currentList = Array.isArray(images) ? images.filter(Boolean) : (images ? [images] : []);
  } else if (multiple) {
    currentList = Array.isArray(values) ? values.filter(Boolean) : [];
  } else if (value) {
    currentList = [value];
  }

  const singlePreview = isImagesMode ? (currentList[0] || '') : (value || currentList[0] || '');

  // Universal change handler
  const emitAdd = (newUrl) => {
    if (!newUrl) return;
    if (isImagesMode) {
      if (maxImages > 1) {
        if (onChange) onChange([...currentList, newUrl]);
      } else {
        if (onChange) onChange([newUrl]);
      }
    } else {
      if (multiple) {
        if (onMultipleChange) onMultipleChange([...values, newUrl]);
      } else {
        if (onChange) onChange(newUrl);
      }
    }
  ...
  const emitRemove = (indexToRemove = 0) => {
    if (isImagesMode) {
      if (maxImages > 1) {
        if (onChange) onChange(currentList.filter((_, i) => i !== indexToRemove));
      } else {
        if (onChange) onChange([]);
      }
    } else {
      if (multiple) {
        if (onMultipleChange) onMultipleChange(values.filter((_, i) => i !== indexToRemove));
      } else {
        if (onChange) onChange('');
      }
    }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1 bg-slate-100 p-0.5 rounded-lg text-[10px] ml-auto">
          <button
            type="button"
      ...
      {activeMode === 'upload' ? (
        ...
        >
          ...
          {uploading ? (
            <div className="py-3 flex flex-col items-center justify-center gap-2 text-purple-600">
            ...
            <div className="py-2 flex flex-col items-center justify-center gap-1.5 text-slate-500">
              ...
              <div>
                <p className="text-xs font-bold text-slate-800">
                  {placeholder || 'Kéo và thả ảnh vào đây, hoặc'} <span className="text-purple-600 underline">chọn từ máy tính</span>
                </p>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 text-xs p-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500"
          />
          <button
            type="button"
      ...
      {/* Multiple Preview */}
      {isMulti && currentList && currentList.length > 0 && (
